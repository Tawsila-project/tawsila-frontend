...
setStatus(`Order Status: ${currentStatus}`);
if (currentStatus === "COMPLETED") {
setIsCompleted(true);
setDriverLocation(null);
}
setCustomerLocation(data.customer?.coords || { lat: 34.12, lng: 35.65 });
setDriverLocation((currentStatus?.toLowerCase() === "in_transit" && data.tracked_location?.lat) ? data.tracked_location : null);
} catch {
...
<Typography variant="h6" color="success.main" textAlign={'center'} fontWeight="bold" sx={{ fontSize: { xs: "1rem", sm: "1.25rem" } }}>
✅ Delivery Completed!
</Typography>
) : (
<Typography variant="h6" textAlign={'center'} fontWeight="bold" sx={{fontSize: { xs: "1rem", sm: "1.25rem" } }} >
🚚 Delivery Tracking
</Typography>
);
...
<img src={Logo} alt="Company Logo" style={{ width: 90, height: "90", display: "flex" , marginLeft: "auto", marginRight: "auto", marginBottom: 8}} />
{displayTitle}
...
{!driverLocation && !isCompleted && <LinearProgress sx={{ mt: 1, height: 5, borderRadius: 1 }} />}
</Box>
</Box>

<Box sx={{ height: { xs: 300, sm: 400, md: 450 }, width: "100%", position: "relative" }}>
{!driverLocation && customerLocation && !isCompleted && (
<Box sx={{
...
{isCompleted && (
<Box sx={{
position: 'absolute', zIndex: 1000, top: 0, left: 0, right: 0, bottom: 0,
...
scrollWheelZoom={!isCompleted}
dragging={!isCompleted}
touchZoom={!isCompleted}
doubleClickZoom={!isCompleted}
boxZoom={!isCompleted}
>
...
{driverLocation && !isCompleted && <Marker position={[driverLocation.lat, driverLocation.lng]} icon={driverIcon}><Popup><b>Driver is here!</b></Popup></Marker>}
{driverLocation && customerLocation && !isCompleted && <Polyline positions={[[driverLocation.lat, driverLocation.lng], [customerLocation.lat, customerLocation.lng]]} color="blue" dashArray="10,10" opacity={0.6} />}
